...
function areaAcres(){
  if (drawnFeature) {
    return turf.area(drawnFeature) * ACRE_PER_SQ_METER;
  }
...
function grossValue(acres){
  if (!mixedMode) {
    ...
    return acres * c.value_per_acre_year * selectedQuality;
  }
  let gross = 0, totalPct = 0;
...
function update(){
  ...
  if (!mixedMode) {
    document.getElementById('plainAnswer').innerHTML =
      `<strong>${fmtAc(acres)}</strong> of <strong>${c.label.toLowerCase()}</strong> is estimated at <strong>${fmtMoney(gross)} per year</strong> before optional costs. This uses a <strong>${Math.round(selectedQuality*100)}%</strong> health score and a source value of <strong>${fmtMoney(c.value_per_acre_year)}/acre/year</strong>.`;
  } else {
    document.getElementById('plainAnswer').innerHTML =
...
function buildMixRows(){
  ...
  if (selects[1]) selects[1].value = 'turf_proxy';
